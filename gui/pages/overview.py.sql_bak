...
def _card(
    content_html: str, border_top_color: str = None, extra_style: str = ""
) -> str:
...
def _label(text: str) -> str:
    t = _tok()
    return (
        f"<div style='font-size:10px;font-weight:500;color:{t['t3']};"
...
def render_model_behavior_sidebar():
    ...
    _sel_task_ov = st.selectbox(
        "Filter task", ["all"] + sorted(_task_list), key="ov_task_filter"
    )
    _cmp_filtered = (
        _model_cmp
        if _sel_task_ov == "all"
...
def render(ctx: dict):
    ...
    if not _live_jobs.empty:
        _has_running = any(
            s in ("running", "started") for s in _live_jobs.status.values
        )
        if _has_running:
            import time as _t
        ...
        for _, _jrow in _live_jobs.iterrows():
            ...
            try:
                ...
                _te = (
                    _dt.datetime.fromisoformat(_jend_s.replace("Z", ""))
...
.kpi-sub  { font-size:8px; color:#475569; margin-top:1px; }
    ...
    with _kc1:
        st.markdown(
            _kpi_card(
                ov.get("total_runs", "—"), "Total Runs", "experiments run", "#22c55e"
            ),
            unsafe_allow_html=True,
        )
    with _kc2:
        st.markdown(
            _kpi_card(
                f"{ov.get('total_energy_j',0) or 0:.1f}J",
                "Total Energy",
                "all runs measured",
                "#f59e0b",
            ),
    ...
    with _kc6:
        st.markdown(
            _kpi_card(
                f"{ov.get('avg_carbon_mg',0) or 0:.3f}mg",
                "Avg Carbon",
                "avg per run",
                "#34d399",
            ),
    ...
    try:
        ...
        _nob = int(
            _q1("SELECT COUNT(*) AS n FROM runs WHERE baseline_id IS NULL").get("n", 0)
            or 0
        )
        _nsy = int(
            _q1(
        ...
        _suf = (
            _q1(
                """SELECT COUNT(*) AS total_cells, SUM(CASE WHEN run_count>=30 THEN 1 ELSE 0 END) AS sufficient_cells FROM (SELECT COUNT(*) AS run_count FROM runs r JOIN experiments e ON r.exp_id=e.exp_id WHERE e.model_name IS NOT NULL AND e.task_name IS NOT NULL AND e.workflow_type IS NOT NULL GROUP BY r.hw_id,e.model_name,e.task_name,e.workflow_type)"""
            )
            or {}
        )
        _sp = round(
        ...
        _chips = (
            _chip(
                _inv,
                "invalid",
                "#ef4444" if _inv > 0 else "#22c55e",
                "#2a0c0c" if _inv > 0 else "#052e1a",
            )
            + _chip(
                _thr,
                "throttled",
                "#f97316" if _thr > 0 else "#22c55e",
                "#2a1000" if _thr > 0 else "#052e1a",
            )
            + _chip(
                _nob,
                "no baseline",
                "#f59e0b" if _nob > 0 else "#22c55e",
                "#2a1a00" if _nob > 0 else "#052e1a",
            )
            + _chip(
                _nsy,
                "noisy env",
                "#a78bfa" if _nsy > 0 else "#22c55e",
                "#1a0e40" if _nsy > 0 else "#052e1a",
            )
            + _chip(
                f"{_sp}%",
                "sufficient",
                "#22c55e" if _sp >= 80 else "#f59e0b" if _sp >= 40 else "#ef4444",
                "#052e1a" if _sp >= 80 else "#2a1a00" if _sp >= 40 else "#2a0c0c",
            )
        )
        st.markdown(
            f"<div style='display:flex;align-items:center;padding:8px 14px;background:{t['bg2']};border:0.5px solid {t['brd']};border-radius:8px;margin-bottom:12px;'><span style='font-size:9px;font-weight:700;color:{t['t3']};text-transform:uppercase;letter-spacing:.1em;margin-right:12px;flex-shrink:0;'>Data Health</span>{_chips}</div>",
            unsafe_allow_html=True,
        )
    except Exception:
        pass
    ...
    def _fixed_card(
        body_html: str, footer_html: str, top_color: str = "#3b82f6"
    ) -> str:
    ...
    try:
        ...
        if not _sess_df.empty:
            for _, _sr in _sess_df.iterrows():
                ...
                _pill = (
                    f"<span style='font-size:9px;padding:1px 5px;"
                ...
                try:
                    ...
                    _ts = (
                        f"{_s//3600}h"
                        if _s > 3600
                        else f"{_s//60}m" if _s > 60 else "now"
                    )
                except Exception:
                    _ts = "—"
    ...
    for _i in range(6 - len(_sess_real_rows)):
        _c3_body += _empty_row(
            "no sessions yet" if _i == 0 and not _sess_real_rows else ""
        )
    _c3_foot = (
    ...
    with col_bd:
        ...
        st.markdown(
            _card(_bd_content, border_top_color="#3b82f6"), unsafe_allow_html=True
        )
    ...
    with col_attr:
        ...
        st.markdown(
            _card(_attr_content, border_top_color="#ef4444"), unsafe_allow_html=True
        )
    ...
    with col_tl:
        ...
        for _bi in range(_bars):
            ...
            _clr_bar = (
                "#d97706"
                if _x < plan_pct / 100
                else "#3b82f6" if _x < (plan_pct + exec_pct) / 100 else "#7c3aed"
            )
        ...
        st.markdown(
            _card(_tl_content, border_top_color="#3b82f6"), unsafe_allow_html=True
        )
    ...
    with col_hw:
        ...
        def _ok(v):
            return "#16a34a" if v else t["t3"]

        _hw_html = (
            _hw_row("CPU", hw.get("cpu_model", "—"))
            + _hw_row(
                "Arch",
                f"{hw.get('cpu_architecture','—')} · "
                f"{hw.get('cpu_vendor','—')} · "
            ...
            + _hw_row(
                "ISA",
                f"AVX2:{'✓' if hw.get('has_avx2') else '✗'}  "
        ...
        _hw_html += (
            f"<div style='font-size:9px;color:{t['t3']};margin-bottom:6px'>"
    ...
    with col_env:
        ...
        _env_html = (
            _env_row(
                "Python",
                f"{env.get('python_version','—')} · {env.get('python_implementation','—')}",
            )
            + _env_row(
                "OS", f"{env.get('os_name','—')} {env.get('kernel_version','—')}"[:32]
            )
            ...
            + _env_row(
                "Torch", env.get("torch_version", "—") or "not installed", t["t3"]
            )
    ...
    with col_act:
            ...
            if not _act_df.empty:
                for _, _ar in _act_df.iterrows():
                    ...
                    try:
                        ...
                        _ats = str(
                            _ar.get("completed_at") or _ar.get("started_at") or ""
                        )
                        _ago = _dt.datetime.now() - _dt.datetime.fromisoformat(
                            _ats.replace("Z", "")
                        )
                        ...
                        _ts = (
                            f"{_s//3600}h ago"
                            if _s > 3600
                            else f"{_s//60}m ago" if _s > 60 else "now"
                        )
                    except Exception:
                        _ts = "—"
                    _body = (
        ...
        if not _act_html:
            _act_html = (
                f"<div style='font-size:10px;color:{t['t3']}'>No activity yet</div>"
            )

        st.markdown(
            _card(f"{_label('Activity feed')}{_act_html}", border_top_color="#16a34a"),
            unsafe_allow_html=True,
        )
