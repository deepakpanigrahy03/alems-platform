...
CREATE TABLE IF NOT EXISTS metric_display_registry (
  ...
  unit_options          TEXT,       -- JSON array: ["µJ","mJ","J","Wh"]
  unit_scales           TEXT,       -- JSON: {"µJ":1,"mJ":0.001,"J":0.000001}
  ...
  visible_in            TEXT        DEFAULT '["workbench"]', -- JSON array
  ...
  created_at            REAL        DEFAULT (unixepoch()),  -- Unix timestamp seconds
  updated_at            REAL        DEFAULT (unixepoch())   -- Unix timestamp seconds
);
 
-- View: metric_display_registry + formula from method registry
-- Use this in PageRenderer instead of querying mdr directly
CREATE VIEW IF NOT EXISTS metric_display_full AS
SELECT
  mdr.*,
  mmr.formula_latex,
  mmr.code_snapshot,
...
CREATE TABLE IF NOT EXISTS page_templates (
  ...
  description  TEXT,
  audience     TEXT DEFAULT '["workbench"]',
  -- JSON array of section definitions (component+props, no data)
  sections_template TEXT,
  created_at  REAL  DEFAULT (unixepoch())
);
